...
namespace ConsoleApplication1
{
    class Program
    {
        ...
        public static int FindValue(string wire)
        {
            int val = 0;
            if(int.TryParse(wire, out val))
            {
                return val;
            }

            if (wires.TryGetValue(wire, out val))
            {
                return val;
            }

            string order = "";
            foreach(string str in arrInputs)
            {
            ...
            string[] splitOrder = order.Split(new string[] { " -> " }, StringSplitOptions.None);
 
            string left = splitOrder[0];

            if (left.Contains("NOT"))
            {
                int wireVal = 0;
                wireVal = ~FindValue(left.Replace("NOT ", ""));
                wires.Add(wire, wireVal);
                return wireVal;
            }
            else if (left.Contains("LSHIFT"))
            {
                string[] splitStr = left.Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);

                int shiftVal = int.Parse(splitStr.Last());

                int wireVal = FindValue(splitStr[0]);
                wires.Add(wire, wireVal << shiftVal);
                return wireVal << shiftVal;
            }
            ...
            else if (left.Contains("AND"))
            {
                ...
                int wireVal = FindValue(splitStr[0]) & FindValue(splitStr[2]);
